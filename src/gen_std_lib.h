 ...
 **************************************************************************/
 ...
 #define _GEN_STD_LIB_H
 
 /* ---------------------- macro defination ------------------------------------------------ */ 

 /* Bit Operation macros */
     /* Set bit pos  in byte data   */
#define Set_Bit_in_Data(data_num , bit_pos)                         (((data_num)) |=  (1 << (bit_pos)))     
      /* Clear bit pos in byte data */ 
#define Clear_Bit_in_Data(data_num ,bit_pos)                         (((data_num)) &= (~(1 << (bit_pos))))      
    /* flip bit pos in byte data  */ 
#define Toggle_Bit_in_Data(data_num , bit_pos)                       (((data_num)) ^=  (1 << (bit_pos)))        
    /* Test if bit pos in byte data  is set   */
#define Test_Bit_Is_Set_in_Data(data_num ,bit_pos)                    ((((data_num)) & (1 << (bit_pos))) >> (bit_pos))      
   /* Test if bit pos in byte data is clear */  
#define Test_Bit_Is_Clear_in_Data(data_num ,bit_pos)                  (!((((data_num)) & (1 << (bit_pos))) >> (bit_pos))) 
