...
static int16_t Print_Real_Num(char **const out_str_ptr, const real_num_t print_real_num, const uint8_t conf_width_spec, const uint8_t conf_num_digits_after_point, const uint16_t ctrl_flag );
...
{ 
    ...
    { 
		...
		for(i = 1; i <=num_digits_after_point; ++i)
		{
	        ...
	        {
	        	case FLOAT_FIXED_POINT_TYPE:
				   extract_real_num_power_1.float_num = frac_part.float_num * (float)Power_Of(BASE_10, 1);
				   int_part = (int32_t)extract_real_num_power_1.float_num;
		        break;
		        case DOUBLE_FIXED_POINT_TYPE:
		           extract_real_num_power_1.double_num = frac_part.double_num * (double)Power_Of(BASE_10, 1);
				   int_part = (int32_t)extract_real_num_power_1.double_num;
		        break;
		        case LONG_DOUBLE_FIXED_POINT_TYPE:
		           extract_real_num_power_1.long_double_num = frac_part.long_double_num * (long double)Power_Of(BASE_10, 1);
...
uint16_t Str_To_Real_Num_Conv(real_num_t *const real_num_ptr, const char *const real_num_in_str_ptr, const uint8_t real_val_type, uint8_t num_digits_after_point) 
{
	 ...
	 {
		 case FLOAT_FIXED_POINT_TYPE:
		     real_num_ptr->float_num = (float)0.0; 
		 break;
		 case DOUBLE_FIXED_POINT_TYPE:
		    real_num_ptr->double_num = (double)0.0; 
		 break;
		 case LONG_DOUBLE_FIXED_POINT_TYPE:
		     real_num_ptr->long_double_num = (long double)0.0; 
		 break;
		 default:
		    system_status_flag =  ERR_FORMAT_INVALID;
	 ...
	 for(i = num_digits_int_or_frac_part + 1; i <= num_digits_after_point; ++i)
	 {
		 switch(real_val_type)
	     {
	     	  case FLOAT_FIXED_POINT_TYPE:
		          real_num_ptr->float_num += (float)(0 * Power_Of(BASE_10, -i));
		      break;
		      case DOUBLE_FIXED_POINT_TYPE:
		          real_num_ptr->double_num += (double)(0 * Power_Of(BASE_10, -i)); 
		      break;
		      case LONG_DOUBLE_FIXED_POINT_TYPE:
		          real_num_ptr->long_double_num += (long double)(0 * Power_Of(BASE_10, -i));
		      break;
	     }		
	 }
	 switch(real_val_type)
	...
	for (; *transverse_ptr != NULL_CHAR; ++transverse_ptr)
	{
		if (*transverse_ptr == '%')
		{
			...
			if(*transverse_ptr == 'f' )
			{
				...
				if((ret_status = Print_Real_Num(&out_stream_buf_ptr, real_num_print_data, width_spec, num_digits_after_point, BASE_10 | STATE_YES << SIGN_FLAG_BIT_POS | pad_format << PAD_FORMAT_BIT_POS | NUM_CONV_ALPHA_NA << ALPHA_BIT_POS | FLOAT_FIXED_POINT_TYPE << REAL_NUM_TYPE_BIT_POS)) == 0)
				{
					system_status_flag = ERR_DEV_OPER_WRITE_FUNC;
