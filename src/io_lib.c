...
uint16_t IO_Channels_Func_Set(const io_config_t *const cur_io_config_ptr)
{
	 ...
	 if(cur_io_config_ptr == NULL_DATA_PTR )
	 {
		 ...
		 return system_status_flag; 
	       ...
	       if((Config_Port_Pin_Range_Validate(cur_io_config_ptr )) != SUCCESS)
			{
				   system_status_flag = ERR_IO_PIN_RANGE_INVALID;
	   ...
	   // common to all IO pin's function process 
	  	...
	  	{
		     case IO_FUNC_GPIO:
		        if((Config_GPIO_Func_Set(cur_io_config_ptr)) != SUCCESS)
		        {
       ...
       case PORT_CH_01:
				 switch(port_pin)
				 {
	  	          ...
	  	          {
		               case IO_FUNC_TRACE:
										 if((Trace_IO_Func_Set_Validate(cur_io_config_ptr)) != SUCCESS)
										 {
             ...
             case PIN_26:
	  	          ...
	  	          {
		               case IO_FUNC_DEBUG:
										 if((Debug_IO_Func_Set_Validate(cur_io_config_ptr)) != SUCCESS)
										 {
										   system_status_flag = ERR_TRACE_FUNC_SET;
										    ret_status = system_status_flag;
...
uint16_t IO_Channel_Write(const uint8_t io_channel, const uint8_t data)
{
	...
	switch(data)
	{
		case STATE_HIGH:
         ...
         default:
		    #ifdef TRACE_ERROR
			    Print("ERR: IO_Write data : %u\r", data);
			#endif
...
uint16_t IO_Channel_Read(const uint8_t io_ch, uint8_t *const read_data_ptr)
{
	 ...
	 if((Check_GPIO_Func(io_ch, &io_ch_read)) != SUCCESS)
	 {
...
static uint16_t Config_GPIO_Func_Set(const io_config_t *const cur_io_config_ptr)
{
	  ...
	  if((GPIO_Func_Set(cur_io_config_ptr->io_ch, cur_io_config_ptr->port_pin_len)) != SUCCESS)
	  {
			system_status_flag = ERR_GPIO_FUNC_SET;
	  ...
	  switch(cur_io_config_ptr->signal)
	  {
		   case PIN_SIG_DIGITAL:
		     ...
		     {
				  case IO_DIR_INPUT:
				     if(cur_io_config_ptr->state != STATE_HIGH && cur_io_config_ptr->state != STATE_LOW)
					 {
				     ...
				     for(i = 0; i < cur_io_config_ptr->port_pin_len ; ++i)
                      ...
                      if(cur_io_config_ptr->state == STATE_HIGH)
					  {
						  for(i = 0; i < cur_io_config_ptr->port_pin_len; ++i)
						 	{
            ...
            else if(cur_io_config_ptr->state == STATE_LOW)
            {						 
				for(i = 0; i < cur_io_config_ptr->port_pin_len; ++i)
				{
              ...
              break;		
							case IO_DIR_OUTPUT:
							    ...
							    if(cur_io_config_ptr->state == STATE_LOW)
								{
									    for(i = 0; i < cur_io_config_ptr->port_pin_len; ++i)
									  	{
												*io_clr_ptr[port] = 1 << (start_port_pin + i);
                                ...
                                else if(cur_io_config_ptr->state == STATE_HIGH) 								
								{		
									for(i = 0; i < cur_io_config_ptr->port_pin_len; ++i)
									{
									    *io_set_ptr[port] = 1 << (start_port_pin + i);
					 ...
					 default: 
					   system_status_flag = ERR_PIN_SIGNAL;
					    Error_or_Warning_Proc("03.06.07", ERROR_OCCURED, system_status_flag);
                       return system_status_flag;					    
					  	// break;										 
		 }	     		 
...
uint16_t Config_Port_Pin_Range_Validate(const io_config_t *const cur_io_config_ptr )
{
	...
	if((IO_Ch_Validate(cur_io_config_ptr->io_ch, cur_io_config_ptr->port_pin_len )) != SUCCESS)
	{
		system_status_flag = ERR_IO_PIN_RANGE;
	...
	if((Config_Consucc_Bits(FLAG_CONSUCC_BITS_1, &set_port_pins)) != SUCCESS)
	{
...
uint16_t IO_Ch_Func_Reset(const io_config_t *const io_unconfig_ptr)
{
	...
	if((IO_Ch_Validate(io_unconfig_ptr->io_ch, io_unconfig_ptr->port_pin_len)) != SUCCESS)
	 {
   ...
   if((Config_Consucc_Bits(FLAG_CONSUCC_BITS_1, &mask_io_ch)) != SUCCESS )
	 {
		 system_status_flag = ERR_IO_CH_UNCONFIG;
	 ...
	 {
		   if((SW_Ch_UnConfig_By_IO_Ch(io_unconfig_ptr->io_ch, io_unconfig_ptr->port_pin_len)) != SUCCESS)
       {
	 ...
	 if((GPIO_Func_Set(io_unconfig_ptr->io_ch, io_unconfig_ptr->port_pin_len)) != SUCCESS)
	 {
		 system_status_flag = ERR_IO_CH_UNCONFIG;
...
static uint16_t Config_Port_Pin_Validate(const uint8_t io_ch)
{
	...
	if((IO_Ch_Validate(io_ch, 1)) != SUCCESS)
	{
		system_status_flag = ERR_IO_PIN_RANGE;
...
uint16_t Check_GPIO_Func(const uint8_t io_channel, const void *const data_ptr)
{
   consucc_bit_t *consucc_bit_ptr = (consucc_bit_t *)data_ptr;
	 ...
	 if((IO_Ch_Validate(io_channel, consucc_bit_ptr->bits_len)) != SUCCESS)
	 {
	     system_status_flag = ERR_CHECK_GPIO_FUNC;
...
uint16_t Non_GPIO_Func_Set(const uint8_t io_ch, const uint8_t func_mode)
{
	...
	if((Config_Port_Pin_Validate(io_ch)) != SUCCESS)
	{
		system_status_flag = ERR_NON_GPIO_FUNC_SET;
...
uint16_t Output_Dev_Init(uint8_t base_io_ch, const uint8_t num_io_chs)
{
	...
	output_config.port_pin_len = num_io_chs;	 
	 if((IO_Channels_Func_Set(&output_config)) != SUCCESS)
	 {
		system_status_flag = ERR_GPIO_FUNC_SET;
...
uint16_t Output_Dev_DeInit(uint8_t base_io_ch, const uint8_t num_io_chs)
{
	...
	output_unconfig.port_pin_len = num_io_chs;	
	 if((IO_Ch_Func_Reset(&output_unconfig)) != SUCCESS)
	 {
